/**
 * CoachUpdatesHistoryDialog.tsx — Fase 6
 *
 * Espelha `ProtocolChangeHistoryDialog` (lado coach), mas para o aluno logado.
 * Lista TODAS as atualizações do coach (`protocol_change_events` do
...
export default function CoachUpdatesHistoryDialog({
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["student-coach-updates-history", studentId],
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Histórico de atualizações do coach</DialogTitle>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 text-sm text-muted-foreground">
            Nenhuma atualização do coach registrada ainda.
          </div>
        ) : (
              ...
              return (
                <div key={row.id} className="rounded-xl border border-border bg-card p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <p className="text-sm font-semibold text-foreground">{fmt(row.created_at)}</p>
                    {row.seen_at ? (
