...
SELECT
  da.id AS source_id,
  da.trainer_id AS coach_id,
  da.student_id,
  'reminder'::text AS source,
  CASE WHEN da.target_date IS NOT NULL AND da.target_date <= CURRENT_DATE
       THEN 'warning' ELSE 'info' END AS severity,
  'Lembrete'::text AS title,
  da.message,
  NULL::text AS suggested_action,
  COALESCE(da.target_date::timestamptz, da.created_at) AS reference_at
